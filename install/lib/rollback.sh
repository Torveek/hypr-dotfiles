...
if [[ ! "$BACKUP_DIR" =~ ^"$BACKUP_ROOT" ]] && [[ ! "$BACKUP_DIR" =~ ^/home/[^/]+/\.local/share/hypr-dotfiles/dotfiles-backup ]]; then
  log_warning "Backup is not in expected location"
  log_warning "Expected: $BACKUP_ROOT or ~/.local/share/hypr-dotfiles/dotfiles-backup"
